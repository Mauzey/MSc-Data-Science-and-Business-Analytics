# LinkedIn Learning - Python for Students
# Exercise: 'First Python Project'

# import
import turtle

BACKGROUND_COLOR = '#9EC388'
CRUST_COLOR = '#ECA84F'
SAUCE_COLOR = '#AD0509'
CHEESE_COLOR = '#FBC70F'

PEPPERONI_LOCATIONS = [[-70, 105], [-85, 175], [-25, 50], [-15, 100], [-25, 150], [-30, 205],
                       [15, 50], [20, 120], [20, 200], [60, 156], [71, 215], [80, 90], [95, 150]]

# initialize and configure turtle environment
screen = turtle.Screen()
screen.bgcolor(BACKGROUND_COLOR)
screen.title('My Pizza')

# initialize and configure turtle object
my_turtle = turtle.Turtle()
my_turtle.pensize(5)
my_turtle.shape('circle')


def draw_circle(radius, line_color, fill_color):
    """ Draws a circle of specified radius
    :param radius: Radius of the circle to be drawn
    :param line_color: Colour of the circle outline
    :param fill_color: Colour of the circle fill
    """
    my_turtle.color(line_color)
    my_turtle.fillcolor(fill_color)
    my_turtle.begin_fill()
    my_turtle.circle(radius)
    my_turtle.end_fill()


def move_turtle(x_pos, y_pos):
    """ Move turtle to specified position
    :param x_pos: X coordinate to travel to
    :param y_pos: Y coordinate to travel to
    """
    my_turtle.up()
    my_turtle.goto(x_pos, y_pos)
    my_turtle.down()


# draw the crust, sauce, and cheese
draw_circle(150, CRUST_COLOR, CRUST_COLOR)
move_turtle(0, 25)
draw_circle(125, SAUCE_COLOR, CHEESE_COLOR)

# draw pepperonis
for loc in PEPPERONI_LOCATIONS:
    move_turtle(loc[0], loc[1])
    draw_circle(10, SAUCE_COLOR, SAUCE_COLOR)

# draw cuts
move_turtle(0, 150)
my_turtle.color(BACKGROUND_COLOR)

for x in range(0, 8):
    my_turtle.pendown()
    my_turtle.left(45)
    my_turtle.forward(150)
    my_turtle.penup()
    my_turtle.backward(150)

# terminate turtle instance
turtle.done()
